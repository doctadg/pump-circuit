...
def main() -> None:
    ...
    report: dict = {'tracks': [], 'boxPickup': {}, 'crateStress': {}, 'camera': {}, 'collision': {}, 'manualDrive': {}, 'steering': {}, 'viewport': {}, 'errors': []}
    ...
    for idx, name in enumerate(track_names):
        ...
        if idx == 0:
            ...
            cdp.evaluate("Object.assign(window.__pumpKart.racers[0],{s:.0948,lane:0,speed:0,worldYaw:null,yawRate:0,item:null,roulette:0,boxLock:0});true")
            time.sleep(.35)
        ...
        cdp.evaluate(f'window.__pumpKart.racers[0].s={focus_s[idx]};window.__pumpKart.racers[0].speed=0;window.__pumpKart.racers[0].worldYaw=null;window.__pumpKart.racers[0].yawRate=0;true')
        time.sleep(0.9)
    ...
    wait_until(cdp, "window.__pumpKart.mode==='race'", timeout=30)
    cdp.evaluate('window.__pumpKart.racers[0].s=.42;window.__pumpKart.racers[0].speed=28;window.__pumpKart.racers[0].worldYaw=null;window.__pumpKart.racers[0].yawRate=0;true')
    time.sleep(0.8)
    ...
    if report['camera']['fov'] != 58 or report['camera']['kartScale'] != .64 or report['camera']['fxDrawCalls'] != 3 or report['camera']['version'] != 'pump-kart-manual-steering-v11':
